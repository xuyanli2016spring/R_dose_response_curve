...
plot_ly(data1, x = ~mouse_doses, y = ~mouse_response, name = "mouse", type = "scatter", mode = "lines", marker = list(color = "blue")) %>%
  add_trace(data2, x = ~culture_doses, y = ~culture_response, type = "scatter", name = "culture", mode = "lines", marker = list(color = "red")) %>%
  layout(title = "mouse and culture dose response curve", 
         xaxis = list(title = "Dose", showgrid = T),
         yaxis = list(title = "Nectrig Rate")
  )
